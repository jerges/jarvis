...
class OrchestratorAgentTest {
    ...
    void processRoutesToCorrectAgent() {
        ...
        when(developerAgent.process(anyString(), anyString(), any())).thenReturn("Java response");
    ...
    void processGeneratesUuidWhenConversationIdIsNull() {
        ...
        stubRoutingDecision(null, decision);
        when(chatClientFactory.defaultProvider()).thenReturn(ModelProvider.ANTHROPIC);
        when(developerAgent.process(anyString(), anyString(), any())).thenReturn("response");

        orchestrator.process(new AgentRequest("message", null, null));

        var captor = ArgumentCaptor.forClass(String.class);
        verify(developerAgent).process(anyString(), captor.capture(), any());
    ...
    void processUsesProvidedConversationId() {
        ...
        verify(developerAgent).process(anyString(), anyString(), any());
        var captor = ArgumentCaptor.forClass(String.class);
        verify(developerAgent).process(anyString(), captor.capture(), any());
    ...
    void processUsesDefaultProviderWhenRequestProviderIsNull() {
        ...
        stubRoutingDecision(null, decision);
        when(chatClientFactory.defaultProvider()).thenReturn(ModelProvider.ANTHROPIC);
        when(devopsAgent.process(anyString(), anyString(), any())).thenReturn("k8s answer");
    ...
    @Test
    void streamDelegatesToCorrectAgent() {
        ...
        assertThat(result.collectList().block()).containsExactly("token1", "token2");
    }
